...
const ChartDeaths = ({ countrie }) => {
  ...
  const dataCharts = (history) => {
    ...
    for (let [k] of Object.entries(history?.deaths || "")) {
      let options = {
        year: "numeric",
        month: "long",
        day: "numeric",
    ...
    for (let [k, v] of Object.entries(history?.deaths || "")) {
      deaths.push(v);
    }
  ...
  return (
    <div className="border mt-6 border-[#404040] px-4 py-3 md:px-10 md:py-5 rounded-xl">
      <h1 className="text-2xl mb-2">Meninggal</h1>
